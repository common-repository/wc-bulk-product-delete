...
class WooCommerce_Bulk_Product_Delete_Admin_Ajax {
   ...
   public static function check_delete(){ 
       if(isset($_POST['wc-bpd-values']) && ! empty($_POST['wc-bpd-values'])){
            ...
            if(sanitize_text_field($_POST['wc-bpd-value-type']) == 'id'){ $data = self::delete_product_id($ids); }
            if(sanitize_text_field($_POST['wc-bpd-value-type']) == 'sku'){ $data = self::delete_product_sku($ids);}
        } 
    }


    public static function get_value_array(){
        $comma_seperated = explode(',',sanitize_text_field($_POST['wc-bpd-values']),2);
        if(count($comma_seperated) == 2){
            return explode(',',sanitize_text_field($_POST['wc-bpd-values']));
        } else {
            return explode(PHP_EOL,sanitize_text_field($_POST['wc-bpd-values']));
        }
    }
    ...
    public static function delete_product_id($ids){
        ...
        if(sanitize_text_field($_POST['wc-bpd-delete-type']) == 'delete'){
